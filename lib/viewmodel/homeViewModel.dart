  ...
  class HomeViewModel extends ChangeNotifier {
    ...
    final UserHistoryRepository _historyRepository = UserHistoryRepository();
    ...
    Future<void> fetchCalorieForMeal() async {
      try {
        ...
        if (user != null) {
          ...
          // Ambil kalori untuk tiap jenis meal
          breakfastCalorie = await _historyRepository.getCalorieConsumedForMeal(user.uid, "breakfastCalorie");
          lunchCalorie = await _historyRepository.getCalorieConsumedForMeal(user.uid, "lunchCalorie");
          dinnerCalorie = await _historyRepository.getCalorieConsumedForMeal(user.uid, "dinnerCalorie");
          snackCalorie = await _historyRepository.getCalorieConsumedForMeal(user.uid, "snackCalorie");
          totalCalorieConsumed = await _historyRepository.getCalorieConsumedForMeal(user.uid, "totalCalorieConsumed");


          print('Breakfast Calories: $breakfastCalorie');
          print('Lunch Calories: $lunchCalorie');
          print('Dinner Calories: $dinnerCalorie');
          print('Snack Calories: $snackCalorie');
          print('Snack Calories: $totalCalorieConsumed');
        } else {
          print('No user logged in');
        }
      } catch (e) {
        print('Error fetching calorie for meal: $e');
      } finally {
        _isLoading = false;
        notifyListeners();
      }
    }
    // Method untuk memastikan history harian
    Future<void> ensureDailyHistoryForCurrentUser() async {
      try {
        final User? currentUser = _auth.currentUser;

        if (currentUser != null) {
          await _historyRepository.ensureDailyHistory(currentUser.uid);
          print('History ensured for user ${currentUser.uid}');
        } else {
          print('No user logged in. Cannot ensure daily history.');
        }
      } catch (e) {
        print('Error ensuring daily history: $e');
      }
    }

    Future<void> initializeHomepage() async {
      // Pastikan history harian
      await ensureDailyHistoryForCurrentUser();
      // Lakukan proses inisialisasi lainnya
      await fetchSurveyDetails();
      await fetchCalorieForMeal();
    }
